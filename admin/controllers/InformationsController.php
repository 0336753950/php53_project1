<?php 
	...
	class InformationsController extends Controller{
		...
		public function update(){
			...
			$action = "index.php?controller=informations&action=updatePost&id=$id";
			$record = $this->modelGetRecord($id);
			//goi view
			$this->loadView("InformationsForm.php",["record"=>$record,"action"=>$action]);
		}
		//update - POST
		public function updatePost(){
			$id = isset($_GET["id"])&&is_numeric($_GET["id"])?$_GET["id"]:0;
			$this->modelUpdate($id);
			//quay tro lai mvc informations
			header("location:index.php?controller=informations");
		}
		//create
		public function create(){
			//tao bien action de xuat vao thuoc tinh action cua the form
			$action = "index.php?controller=informations&action=createPost";
			//goi view
			$this->loadView("InformationsForm.php",["action"=>$action]);		
		}
		//crete POST
