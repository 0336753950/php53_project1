<?php 
	...
	class ServicesController extends Controller{
		...
		public function update(){
			...
			$action = "index.php?controller=services&action=updatePost&id=$id";
			$record = $this->modelGetRecord($id);
			//goi view
			$this->loadView("ServicesForm.php",["record"=>$record,"action"=>$action]);
		}
		//update - POST
		public function updatePost(){
			$id = isset($_GET["id"])&&is_numeric($_GET["id"])?$_GET["id"]:0;
			$this->modelUpdate($id);
			//quay tro lai mvc services
			header("location:index.php?controller=services");
		}
		//create
		public function create(){
			//tao bien action de xuat vao thuoc tinh action cua the form
			$action = "index.php?controller=services&action=createPost";
			//goi view
			$this->loadView("ServicesForm.php",["action"=>$action]);		
		}
		//crete POST
