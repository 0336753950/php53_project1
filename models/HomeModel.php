<?php 
	trait HomeModel{
		...
		public function modelGetProductsHome($category_id){
			$conn = Connection::getInstance();
			//lay cac san pham thuoc danh muc do va danh muc cap con cua no
			$query = $conn->query("select * from products where category_id in (select id from categories where id=$category_id or parent_id=$category_id) and hot =1 order by id desc");
			$result = $query->fetchAll();
			return $result;
		}
